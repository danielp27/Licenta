...
cleanup() {
        ...
        if [ -f deploy_nginx_inside_vm.sh ]; then
                rm deploy_nginx_inside_vm.sh
        fi
        exit 1
}
...
# deploy la Nginx cu pagină implicită în clusterul său Kubernetes
cat <<EOF > deploy_nginx_inside_vm.sh
#!/bin/bash
...
scp -i ~/.ssh/id_ed25519 -P $port deploy_nginx_inside_vm.sh fedora@$ip:/home/fedora/
# Face scriptul executabil și îl rulează în interiorul mașinii virtuale
ssh -i ~/.ssh/id_ed25519 -p $port fedora@$ip "chmod +x deploy_nginx_inside_vm.sh && ./deploy_nginx_inside_vm.sh"

# Șterge scriptul de pe host
rm deploy_nginx_inside_vm.sh

# Extrage NodePortul aplicației Nginx
vm_port=$(ssh -i ~/.ssh/id_ed25519 -p $port fedora@$ip "kubectl get svc nginx -o=jsonpath='{.spec.ports[0].nodePort}'")
